...
namespace ExcelLib
{
    class Excel2007 : IExcel
    {
        ...
        public bool Save(string newSheetName, string[,] array)
        {
            try
            {
                ...
                if (newSheetName != null)
                {
                    ...
                    currentSheet = book.Worksheets[newSheetName];//保存表的名字
                }
        ...
        public byte[] GetCellRGB(int Row, int Col)//获取当前cell的背景色
        {
            byte R;
            byte G;
            byte B;
            byte[] rgb = new byte[3] { 255, 255, 255 };
            if (currentSheet.Cells[Row, Col].Style.Fill.BackgroundColor.Rgb == null)
            {
            ...
            var s = currentSheet.Cells[Row, Col].Style.Fill.BackgroundColor.Rgb;
            string s10 = string.Concat(s[6], s[7]);
            string s32 = string.Concat(s[4], s[5]);
            string s54 = string.Concat(s[2], s[3]);
            R = byte.Parse(s10, System.Globalization.NumberStyles.HexNumber);
            G = byte.Parse(s32, System.Globalization.NumberStyles.HexNumber);
            B = byte.Parse(s54, System.Globalization.NumberStyles.HexNumber);
            rgb[0] = B;
            rgb[1] = G;
            rgb[2] = R;
            return rgb;
